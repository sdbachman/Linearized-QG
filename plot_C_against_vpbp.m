...
for outdir = [21:25]
  ...
  load(strcat('../vpbp_array', num2str(outdir), '.mat'))
%   load(strcat('../C2_array', num2str(outdir), '.mat'))
%   load(strcat('../C3_array', num2str(outdir), '.mat'))
%   load(strcat('../C4_array', num2str(outdir), '.mat'))  
   vpbp_array = vpbp_array * 1e-7;
   yay =(diff(C_array) ./ diff(vpbp_array)) .* (vpbp_array(2:end) + vpbp_array(1:end-1))/2;

  figure(1)
  g = plot(log(C_array), 'LineWidth', 2, 'Color', C(count,:));
  clr = get(g, 'Color');
  
  hold on
    g = plot(log(deltaU * vpbp_array), 'LineWidth', 2, 'Color', [0 0 0]);
